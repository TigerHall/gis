...
| 分组           | 图层                                             |
| -------------- | ------------------------------------------------ |
| 全球板块构造   | 全球16大板块、大陆板块、大洋板块、洋中脊、海沟、转换断层、大西洋/印度洋/太平洋转换断层     |
| 洋中脊作用域   | 全球洋壳、大洋域、次大洋域、洋中脊作用域、作用域边界、全球陆壳                       |
| 火山地震与热点 | 火山、热点、大火成岩省 |
| 大型异常区     | LLSVP、Dupal                          |
| 海底矿产资源   | 结核、结壳、热液喷口、热液硫化物                          |
...
- **方式一：文件关联**（PWA 安装后）
  1. 安装 PWA 应用到系统
  2. 在文件管理器中双击 `.geojson` 文件
  3. 应用会自动启动并加载该文件

- **方式二：拖放打开**
  1. 打开应用
  2. 将 `.geojson` 文件拖放到地图页面
  3. 自动加载并显示
